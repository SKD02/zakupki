...
def parse_quantity_value(value: Any):
    """
    Нормализация количества без блокирующей проверки.

    Пустые ячейки, NaN/NaT и значения из объединённых ячеек возвращаются как None,
    а не как 0 и не как NaN. Это убирает падения JSON/валидации при предпросмотре
    и при работе с несогласованными строками.
    """
    if value is None:
        return None
...
def read_application_excel_with_mapping(
    ...
    for row_index, row in enumerate(df.to_dict(orient="records"), start=int(header_row) + 1):
        ...
        quantity_value = parse_quantity_value(quantity)
        supply_start_date, supply_end_date = parse_supply_period(supply_period)
        ...
        items.append({
            "source_row_no": row_index,
            "material_name": str(material_name).strip(),
            "unit": str(unit).strip() if unit is not None and not pd.isna(unit) else None,
            "quantity": quantity_value,
            "work_doc_code": str(work_doc_code).strip() if work_doc_code is not None and not pd.isna(work_doc_code) else None,
            "work_doc_subject": str(work_doc_subject).strip() if work_doc_subject is not None and not pd.isna(work_doc_subject) else None,
            "supply_start_date": supply_start_date,
            "supply_end_date": supply_end_date,
            "raw_payload": raw_payload,
        })
